...
def init_db():
    async def _init_db():
        ...
        try:
            ...
            print(
                "[red]The database used in the connection string doesn't exist. Please create it.[/red]"
            )

    print("[yellow]Are you sure you want to [bold]init-db[/bold]?[/yellow]")
    print(
        f"[red]This will delete all of your data on [bold]{config.postgres_connection_string}[/bold]![/red]"
    )
...
def migrate():
    ...
    print(
        f"[blue]This will affect [bold]{config.postgres_connection_string}[/bold][/blue]"
    )
    print("Type [blue]y[/blue] to continue:")
    ...
    with importlib.resources.path("idv_server", "alembic.ini") as alembic_ini:
        ...
        alembic_cfg.set_main_option(
            "sqlalchemy.url", config.postgres_connection_string.encoded_string()
        )
